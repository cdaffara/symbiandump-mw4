...
static int compareWithCompareFunctionForQSort(const void *a, const void *b)
{
    ...
    JSValue *vb = *(JSValue **)b;
#if PLATFORM(SYMBIAN) 
    //If both the objects are same then no need to compare. 
    //return 0 from compare function meaning objects are equal.     
    if (va == vb) 
        return 0; 
#endif 
    if (va->isUndefined()) {
...
JSValue* ArrayProtoFunc::callAsFunction(ExecState* exec, JSObject* thisObj, const List& args)
{
  ...
  case ToString:
    ...
    if (!thisObj->inherits(&ArrayInstance::info) && thisObj->toString(exec) != "[object DeviceLiwMap]")
