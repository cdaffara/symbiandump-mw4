...
#if PLATFORM(SYMBIAN)
const int MAXIMUMSTMTEXEC = 200000;
#endif
...
#if PLATFORM(SYMBIAN)
  Interpreter* interpreter = exec->lexicalInterpreter();
  interpreter->incStatementCount();

  // abort if script is too big
  if( interpreter->statementCount() > MAXIMUMSTMTEXEC ) {
	  JSObject *error = Error::create(exec, GeneralError, "Script is too big.");
