...
{
    return parseUASheet((const char*)array, sizeof(array));
...
// length, percent, number
    case CSS_PROP_LINE_HEIGHT:
    {
        ...
        if (lineHeight.type() == Fixed)
            lineHeight.setValue(lineHeight.value()-1);
#endif
...
// shorthand properties
    ...
    case CSS_PROP__WEBKIT_MARQUEE:
        ...
        return;
#if PLATFORM(SYMBIAN)
    case CSS_PROP__WAP_MARQUEE_LOOP: {
        HANDLE_INHERIT_AND_INITIAL(marqueeLoopCount, MarqueeLoopCount)
        if (!primitiveValue) {
            // invalid value, set loopcount to 1
            style->setMarqueeLoopCount(1);
            return;
        }
        if (primitiveValue->getIdent() == CSS_VAL_INFINITE)
            style->setMarqueeLoopCount(1); // 1 loop instead of forever
...
#if PLATFORM(SYMBIAN)
    case CSS_PROP__WAP_MARQUEE_SPEED: 
#endif
...
#if PLATFORM(SYMBIAN)
    case CSS_PROP__WAP_MARQUEE_STYLE:
#endif    
...
#if PLATFORM(SYMBIAN)
    case CSS_PROP__WAP_MARQUEE_DIR: {
...
#if PLATFORM(SYMBIAN)
    case CSS_PROP__WAP_INPUT_FORMAT: {
        HANDLE_INHERIT_AND_INITIAL(wapInputFormat, WapInputFormat)
        if (!primitiveValue) return;
        style->setWapInputFormat(primitiveValue->getStringValue());
        break;
        }
    case CSS_PROP__WAP_INPUT_REQUIRED: {
        HANDLE_INHERIT_AND_INITIAL(wapInputRequired, WapInputRequired)
        if (!primitiveValue) return;
        style->setWapInputRequired(primitiveValue->getIdent() == CSS_VAL_TRUE ? true : false);
        break;
        }
    }
#endif
...
#if PLATFORM(SYMBIAN)
    if( !isAbsoluteSize )
        zoomedSize *= m_document ? m_document->frame()->bridge()->textZoomPercent() / 100.0f : 1.0f;
#endif
