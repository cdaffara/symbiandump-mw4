    ...
    if (!m_accessKeyMapValid) {
        ...
        for (Node* n = firstChild(); n; n = n->traverseNextNode()) {
            if (!n->isElementNode())
                continue;
            Element* element = static_cast<Element*>(n);
            const AtomicString& directKey = element->getAttribute(directkeyAttr);
            if (!directKey.isEmpty())
                m_elementsByAccessKey.set(directKey.impl(), element);
        }
#endif                
...
void Document::write(const String& text)
{
...
#if PLATFORM(SYMBIAN)
    if (isHTMLDocument()) {
        HTMLTokenizer* tokenizer = static_cast<HTMLTokenizer*>(m_tokenizer);
        if(tokenizer)
           tokenizer->setExecutingDocumentWrite(true);
    }
#endif    
    ...
    if (isHTMLDocument()) {
        HTMLTokenizer* tokenizer = static_cast<HTMLTokenizer*>(m_tokenizer);
        if(tokenizer)
           tokenizer->setExecutingDocumentWrite(false);
    }
#endif
...
#if PLATFORM(SYMBIAN)
    } else if (equalIgnoringCase(equiv, "expires") || equalIgnoringCase(equiv, "pragma") || 
              equalIgnoringCase(equiv, "cache-control")) {
        KURL url = frame->loader()->url();
        if ( url.protocol().startsWith("http")) {
            TRAP_IGNORE(StaticObjectsContainer::instance()->resourceLoaderDelegate()->httpSessionManager()->UpdateCacheL( url.url(), equiv.lower(), content));
        }
...
#if PLATFORM(SYMBIAN)
    bool isOverLink = result.URLElement() && 
        (!result.URLElement()->getAttribute(hrefAttr).isNull() || 
        !result.URLElement()->getAttribute(ctiAttr).isNull());
#else
