...
struct MappedAttributeKey {
    ...
    static MappedAttributeKey sDeletedValueKey;
};

MappedAttributeKey MappedAttributeKey::sDeletedValueKey(eLastEntry);
...
struct MappedAttributeKeyTraits : WTF::GenericHashTraits<MappedAttributeKey*> {
    ...
    static MappedAttributeKey* deletedValue() { return &(MappedAttributeKey::sDeletedValueKey); }
};
...
CSSMappedAttributeDeclaration* StyledElement::getMappedAttributeDecl(MappedAttributeEntry entryType, Attribute* attr)
{
    if (!mappedAttributeDecls)
        return 0;
    MappedAttributeKey* key = new MappedAttributeKey(entryType, attr->name().localName().impl(), attr->value().impl());
    CSSMappedAttributeDeclaration* value = mappedAttributeDecls->get( key );
    delete key;
    return value;
}
...
void StyledElement::setMappedAttributeDecl(MappedAttributeEntry entryType, Attribute* attr, CSSMappedAttributeDeclaration* decl)
{
    ...
    mappedAttributeDecls->set(new MappedAttributeKey(entryType, attr->name().localName().impl(), attr->value().impl()), decl);
}
                                                  ...
                                                  const QualifiedName& attrName, const AtomicString& attrValue)
{
    if (!mappedAttributeDecls)
        return;
    MappedAttributeKey* key = new MappedAttributeKey(entryType, attrName.localName().impl(), attrValue.impl());
    mappedAttributeDecls->remove(key);
    delete key;
}
...
unsigned MappedAttributeHash::hash(const MappedAttributeKey* key_)
{
    const MappedAttributeKey& key = *key_;
    uint32_t hash = PHI;
    uint32_t tmp;
