...
HTMLTokenizer::State HTMLTokenizer::parseProcessingInstruction(SegmentedString &src, State state)
{
...
#if PLATFORM(SYMBIAN)    
    bool xmlstylesheet = false;
    String linkbuf("<link rel=\"stylesheet\"");
    
    // Workaround to support <?xml-stylesheet?> PI in XHTML documents in HTML Parser since we choose this parser instead of
    // XML Parser for XHTML documents for some legacy reasons
    if(src.toString().startsWith("?xml-stylesheet"))
    {
        // advance parse buffer tag name "?xml-stylesheet"
        xmlstylesheet = true;
        for (int i=0;i<15;i++) src.advance(lineNumberPtr());
    }
#endif
    ...
    while (!src.isEmpty()) {
        ...
        // we look for an unquoted '>' instead. (IE compatible)
            ...
            if(xmlstylesheet) 
            {
            /* Basically parsing <?xml-stylesheet?> in XHTML as <link> defined in HTML 4.0 which has same semantics including attributes
               except for "alternate" attribute. Refer to http://www.w3.org/TR/xml-stylesheet for details. For now we don't handle "alternate"
               attribute symantics. Put back equivalent <link> tag with atrributes from <?xml-stylesheet?> for parsing.
            */
                linkbuf.replace('?', '>');
                src.prepend(SegmentedString(linkbuf));            
            }
#endif
            return state; // Finished parsing comment!
        }
...
#if PLATFORM(SYMBIAN)    
    OOM_POST_CHECK_FAILED(end();return true;)
#endif    
    return false;
}
