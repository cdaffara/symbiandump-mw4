...
#if PLATFORM(SYMBIAN)
    TReal trg;
    TReal src((end.x() - start.x()) * (end.x() - start.x()) + (end.y() - start.y()) * (end.y() - start.y()));
    Math::Sqrt(trg, src);
    return trg;
#else
...
static float curveLength(PathTraversalState& traversalState, CurveType curve)
{
    ...
    do {
        ...
        if ((length - distanceLine((const FloatPoint&)curve.start, (const FloatPoint&)curve.end)) > kPathSegmentLengthTolerance) {
