           ...
           const xmlChar *ExternalID, const xmlChar *SystemID)
{
     ...
     */
     // TODO: Be sure to perform cleanup in case of OOM or appropriate recovery
    oldinput = ctxt->input;
...
#endif
    if (ctxt->html) {
// TODO: OPTIMIZE: HTML and non-HTML parts here could be combined..
#ifdef LIBXML_HTML_ENABLED
        if (ctxt->myDoc == NULL)
            doc = ctxt->myDoc = htmlNewDocNoDtD(NULL, NULL); // TODO: OOM: review htmlNewDocNoDtD
        if (!ctxt->myDoc)
            goto OOM;
#else
        ...
        if (ctxt->encoding != NULL){
            doc->encoding = xmlStrdup(ctxt->encoding); // TODO: OOM:
            if(! doc->encoding)
                goto OOM;
        }
        //else
    ...
    {
        doc->URL = xmlCanonicPath((const xmlChar *) ctxt->input->filename); // TODO: OOM:
        if (doc->URL == NULL)
            doc->URL = xmlStrdup((const xmlChar *) ctxt->input->filename); // TODO: OOM:
    }
    return;
//----------------------------
OOM:
...
xmlSAX2EndDocument(void *ctx)
{
    ...
    if ((ctxt->inputTab[0]->encoding != NULL) &&
        (ctxt->myDoc != NULL) &&
        (ctxt->myDoc->encoding == NULL))
    {
...
xmlSAX2StartElement(void *ctx, const xmlChar *fullname, const xmlChar **atts)
{
     ...
     */
    // TODO: OPTIMIZE: If we do not use HTML, then simplify IF's condition
    if ((!ctxt->html) &&
    ...
    // TODO: OPTIMIZE: Consider excluding warning reporting at all
...
// DONE: -->nameSpace :TODO: rename "namespace" variable (it matches C++'s keyword)
/**
 * xmlSAX2AttributeNs:
 * @ctx: the user data (XML parser context)
 ...
 * the element.
 */
static void  // TODO: OPTIMIZE: accept pointer to four-item array of values as it comes from parser
                      ...
                      const xmlChar **attributes)
{
    ...
    if (ctxt->freeElems != NULL) {
        ...
        if (ctxt->dictNames)
            ...
            ret->name = xmlStrdup(localname); // TODO: Check OOM state
     ...
     */
        if (nb_attributes > 0) {
            ...
            // TODO: OPTIMIZE: force inlining this method
 ...
 */
// TODO: OPTIMIZE: removing recording Info and other NON-relevant stuff
void
...
xmlSAX2Comment(void *ctx, const xmlChar *value)
{
    ...
    // TODO: OPTIMIZE: combine into one function call, here and in other handlers
    if (ctxt->inSubset == 1) {
        xmlAddChild((xmlNodePtr) ctxt->myDoc->intSubset, ret);
        return;
    } else if (ctxt->inSubset == 2) {
