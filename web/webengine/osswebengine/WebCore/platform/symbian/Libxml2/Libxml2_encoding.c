            ...
            const unsigned char* inb, int *inlenb)
{
    ...
    inlen = *inlenb / 2;
    inend = in + inlen;
    // TODO: OPTIMIZE: prefetch data from golobaState structure
    while ((in < inend) && (out - outstart + 5 < *outlen)) {
        if (xmlLittleEndian) {
        c= *in++;
    } else {
...
xmlAddEncodingAlias(const char* name, const char* alias)
{
    ...
    // TODO: Handle OOM here on in the caller
    return(0);
}
...
//TODO: limit number of encoding handlers -- too much memory allocated
...
xmlNewCharEncodingHandler(const char *name,
    ...
    const char *alias;
    char upper[500]; // TODO: BAD: big buffer on the stack!
    int i;
    char *up = 0;
...
xmlFindCharEncodingHandler(const char *name) {
    ...
    char upper[100];
    int i;
    // 'handlers' is global variable:  TODO: rename it to make it clearer
    if (handlers == NULL){
        xmlInitCharEncodingHandlers();
        if(OOM_FLAG)
            return NULL;
 ...
 *      possible, but reporting is not guaranteed // TODO: FIX OOM reporting
 */
int
...
retry:
    ...
    return(0);
    if (toconv * 2 >= written) {
        xmlBufferGrow(out, toconv * 2); // TODO: Check OOM
        written = out->size - out->use - 1;
    }
    if (handler->output != NULL) {
    ret = handler->output(&out->content[out->use], &written,
...
xmlByteConsumed(xmlParserCtxtPtr ctxt) {
    ...
    if (in->end - in->cur > 0) {
        ...
        // TODO: OPTIMIZED: Big magic numbers for memory allocations!!! Resolve
