...
xmlFatalErr(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char *info)
{
    ...
    switch (error) {
        ...
        case XML_ERR_LITERAL_NOT_FINISHED:
            errmsg = EMBED_ERRTXT("Unfinished System or Public ID \" or ' expected\n");
            break;
...
// XE: REPLACED WITH
    for (i = nr/2; i > 0; i--) {
         ...
         ctxt->nsNr--;
// XE: BEGIN NEW CODE
        // TODO: test endPrefixMapping callback
        /*
...
// XE: END NEW CODE
         ...
         // TODO: XE optimization: do we need to clean up??
    }
    return(nr);
}
#endif
...
// TODO: OPTIMIZE: preevaluate and reuse ctxt->input
#define NEXTL(l) do {                               \
    if (*(ctxt->input->cur) == '\n') {              \
        ctxt->input->line++; ctxt->input->col = 1;  \
    }else{                                          \
    ...
    if ((ctxt->inputNr == 1) && (ctxt->instate != XML_PARSER_DTD)) {
        ...
        */
        ...
        while (IS_BLANK_CH(*cur)) {
            ...
            if (*cur == 0) {
                ctxt->input->cur = cur;
                xmlParserInputGrow(ctxt->input, INPUT_CHUNK); // TODO: OOM:
                cur = ctxt->input->cur;
            }
        }
        ctxt->input->cur = cur;
...
xmlParserHandlePEReference(xmlParserCtxtPtr ctxt) {
    ...
    // TODO: OPTIMIZE: switch statement
    switch(ctxt->instate) {
    ...
    if(OOM_FLAG)
        return;
    // TODO: xmlParserDebugEntities should be #defined and checked by preprocessor
    if (xmlParserDebugEntities)
            ...
            (entity->etype == XML_EXTERNAL_PARAMETER_ENTITY))
    {
        ...
        */
        input = xmlNewEntityInputStream(ctxt, entity); // TODO: OOM: not reviewed yet
        xmlPushInput(ctxt, input);
        ...
        */
        ...
        if ((ctxt->input->end - ctxt->input->cur)>=4) {
            ...
            if (enc != XML_CHAR_ENCODING_NONE) {
                xmlSwitchEncoding(ctxt, enc); // TODO: OOM: not reviewed yet
            }
        }
        ...
        if ((entity->etype == XML_EXTERNAL_PARAMETER_ENTITY) &&
            ...
            (IS_BLANK_CH(NXT(5))))
        {
            xmlParseTextDecl(ctxt);// TODO: OOM: not reviewed yet
        }
    } else {
 ...
 */
 // TODO: OPTIMIZE: make a function -- there are too many references to it
 //       from xmlParseAttValueComplex and xmlStringLenDecodeEntities
...
// TODO: rename local variable 'l' with less error-prone name
/**
...
static int areBlanks(xmlParserCtxtPtr ctxt, const xmlChar *str, int len,
    ...
    if (ctxt->myDoc != NULL) {
        ...
        // TODO: Rather studip write it this way, isn't it??
    ...
    // TODO: OPTIMIZE: preevaluate and recombine conditions
 ...
 */
// TODO: Make sure that in OOM (prefix is set to NULL and freed if needed)
xmlChar*
...
OOM_exit:
    ...
    if (c == ':')
    {
        ...
        *prefix = ret; // NOTE: in 'prefix' we return 'buffer' or a copy from 'buf'
        if (c == 0) {
            return(xmlStrndup(BAD_CAST "", 0)); // TODO: Check OOM, cleanup 'prefix'
        }
        len = 0;
...
xmlParseStringName(xmlParserCtxtPtr ctxt, const xmlChar** str)
{
    ...
    int len = 0, l; // TODO: rename 'l' into something else
    int c;
...
// TODO: OPTIMIZE: define as inline
xmlChar *
    ...
    { /* checked */
        if (len + 5 >= size) {
            ...
            if (!tmp) {
                xmlFree(buf);
                xmlErrMemory(ctxt, NULL);
                ctxt->instate = (xmlParserInputState) state; // TODO: do we need this? in other similar cases it is not this wa
                return(NULL);
            }
            buf = tmp;
        }
 ...
 *
 * returns: the type of element content XML_ELEMENT_TYPE_xxx or NULL if error
 */
...
xmlParseStringEntityRef(xmlParserCtxtPtr ctxt, const xmlChar ** str) {
    ...
    if (name == NULL) {
         ...
         */
        if (ent == NULL) {
            // TODO: OPTIMIZE: Generalize error code (combine)
            if ((ctxt->standalone == 1) ||
 ...
 * [NS 9] ETag ::= '</' QName S? '>'
 */
// TODO: Optimize: 'line' argument can be excluded
static void
...
#ifdef XMLENGINE_XMLINTERFACE
                if (!ctxt->reportXmlnsAsAttribute){
#endif
                    if (alloc != 0)
                        xmlFree(attvalue);
...
failed:
    ...
    if (ctxt->attsDefault != NULL) {
        ...
        if (defaults != NULL) {
            for (i = 0;i < defaults->nbAttrs;i++)
            {
                 ...
                 */
                // TODO: XE optimisation - Combine 1st and 2nd IF cases more efficiently
                if ((attname == ctxt->str_xmlns) && (aprefix == NULL)) {
                    /*
                    * check that it's not a defined namespace
                    */
                ...
                if (aprefix == ctxt->str_xmlns) {
                    ...
                    */
                    ...
                    // DONE: make prefix-to-namespace mappings added to attributes for XmlInterface support
...
#ifdef XMLENGINE_XMLINTERFACE
            const xmlChar* p = atts[i + 1];
            if (ctxt->reportXmlnsAsAttribute   &&
                (*p++ == 'x') && (*p++ == 'm') &&
                (*p++ == 'l') && (*p++ == 'n') && (*p++ == 's'))
            {
                // do nothing: it's a namespace mapping (which is reported for XmlInterface as "attribute")
            }
            else //
#endif
            if(atts[i + 1] != ctxt->str_xmlns)
                xmlNsErr(ctxt, XML_NS_ERR_UNDEFINED_NAMESPACE,
                    EMBED_ERRTXT("Namespace prefix %s for %s on %s is not defined\n"),
    ...
    *URI = nsname;
    ...
    if ((ctxt->sax != NULL) &&
        ...
        (!ctxt->disableSAX))
    {
        // TODO: XE Optimisation: avoid double code for function call
        if (nbNs > 0)
        	{
 ...
 * [NS 9] ETag ::= '</' QName S? '>'
 */
// TODO: OPTIMIZE: 'line' argument is not neccessary
static void
...
xmlParseContent(xmlParserCtxtPtr ctxt) {
    ...
    // TODO: OPTIMIZE: Reorder and optimize checks
    /*
...
xmlParseElement(xmlParserCtxtPtr ctxt) {
    ...
    // TODO: REMOVE: Part of recording mechanism
    if (ctxt->record_info) {
...
xmlParseVersionInfo(xmlParserCtxtPtr ctxt) {
    ...
    xmlChar ch;
    int errCode;
    // TODO: OPTIMIZE: In most places RAW may be optimized greatly by having (prefetching)
    //                 a reference to ctxt->input->cur
    // TODO: OPTIMIZE: CMP7 should be defined via CMP4 and CMP3, where CPM4 for instance compares 4 bytes at once :)
    //                  Problem here is 4-byte alignment for ints
...
xmlCreatePushParserCtxt(xmlSAXHandlerPtr sax, void *user_data,
    ...
    if (enc != XML_CHAR_ENCODING_NONE) {
        xmlSwitchEncoding(ctxt, enc); // TODO: Check OOM flag and do clean up
    }

    return(ctxt);
//------------------------------
OOM_istream_ctxt_buf:
    inputStream->buf = NULL; // NOTE: 'buf' already part of inputStream -- do not free it twice!!!
...
OOM:
    xmlErrMemory(NULL, EMBED_ERRTXT("creating parser: out of memory\n"));
    return(NULL);
}
 ...
 * @URL:  the URL for the entity to load
 * @systemID:  the System ID for the entity to load
                    ...
                    xmlNodePtr *list)
{
    ...
    // TODO: Check: OOM or NOT FOUND (undeclared)
    if (ctxt == NULL)
                             ...
                             const char* filename)
{
    ...
    input = xmlNewInputStream(ctxt);
    if (input == NULL) {
        xmlErrMemory(NULL, EMBED_ERRTXT("parsing new buffer: out of memory\n"));
        xmlFree(ctxt);
        return;
    }
...
// TODO: Create macroses for such "wrappers" or define as inline
...
xmlSAXParseDoc(xmlSAXHandlerPtr sax, xmlChar *cur, int length, int recovery, int* errorCode) {
    ...
    xmlParseDocument(ctxt); // may set OOM flag // REVIEW UNFINISHED
    if(OOM_FLAG){
        xmlFreeDoc(ctxt->myDoc);
        goto cleanup;
    }
...
xmlCtxtReset(xmlParserCtxtPtr ctxt)
{
    ...
    ctxt->disableSAX = 0;
    ctxt->valid = 1;
// TODO: What about this disabled code - do we need it??
#if 0
    ctxt->vctxt.userData = ctxt;
    ctxt->vctxt.error = xmlParserValidityError;
    ctxt->vctxt.warning = xmlParserValidityWarning;
#endif
    // TODO: Remove recording... completely
    ctxt->record_info = 0;
