...
//TODO: Define as macro or inline function
/**
 * xmlIsLetter:
...
xmlNextChar(xmlParserCtxtPtr ctxt)
{
    ...
    if ((*ctxt->input->cur == 0) &&
        (xmlParserInputGrow(ctxt->input, INPUT_CHUNK) <= 0)) // TODO: OOM:
    {
...
xmlCurrentChar(xmlParserCtxtPtr ctxt, int *len) {
    ...
    if (ctxt->charset == XML_CHAR_ENCODING_UTF8) 
    {
        ...
        */
        ...
        c = *cur;
        // TODO: OPTIMIZE: swap branches in this IF (sequential access to the "not" part is faster)
        if (c & 0x80) 
        {
            if (c == 0xC0)
                goto encoding_error;
...
xmlSwitchEncoding(xmlParserCtxtPtr ctxt, xmlCharEncoding enc)
{
    ...
    //TODO: OPTIMIZE: preevaluate and reuse ctxt->input and ctxt->input->cur
    switch (enc) {
                       ...
                       xmlCharEncodingHandlerPtr handler)
{
    ...
    if (input->buf != NULL) {
        ...
        if ((input->buf->buffer != NULL) && (input->buf->buffer->use > 0)) 
        {
            int processed;
            unsigned int use;
            // TODO: OPTIMIZE: preevaluate handler->name -- handler is global!!
            /*
             * Specific handling of the Byte Order Mark for
             * UTF-16
             */
        ...
        if ((input->length == 0) || (input->buf == NULL)) {
            ...
            if(! (input->buf->raw = xmlBufferCreate()))
                return -1; // OOM!
            
            xmlBufferAdd(input->buf->raw, input->cur, input->length - processed);
            if(OOM_FLAG)
                return -1;

            if(! (input->buf->buffer = xmlBufferCreate()))
                return -1; // OOM!
...
xmlNewEntityInputStream(xmlParserCtxtPtr ctxt, xmlEntityPtr entity) {
    ...
    // TODO: OPTIMIZE: xmlParserDebugEntities shouldn't be a global variable -- rather a define
    if (xmlParserDebugEntities)
...
{   // TODO: OOM handling - check OOM condition in all callers
    // TODO: OPTIMIZE: there is too much of repeated code...
    if(ctxt==NULL) {
    ...
    // TODO: "Magic numbers" for Nodes and Names stacks in parser context
    ...
    return(0);
//----------------------------------------------- OOM handling
    // >>: OPTIMIZE: Maybe just clear whole contents of *ctxt ? -- NO, we shouldn't free ctxt
    // TODO: After all, do nothing.. let caller free partially constructed ctxt
 ...
 */
 // TODO: Remove as a part of recording mechanism
void
