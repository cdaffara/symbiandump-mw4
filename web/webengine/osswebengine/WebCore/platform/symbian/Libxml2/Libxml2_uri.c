...
xmlSaveUri(xmlURIPtr uri)
{
    ...
    if (uri->scheme != NULL) {
        p = uri->scheme;
        // TODO: OPTIMIZE: roll two blocks into one do { } while loop
        while (*p != 0) {
            if (len >= max) {
                xmlChar* tmp;
                max *= 2;
    ...
    else
    {
        if (uri->server != NULL) {
            ...
            if (uri->user != NULL) {
                p = uri->user;
                // TODO: OPTIMIZE: roll two IFs into one loop again
                while (*p != 0) {
                    if (len + 3 >= max) {
                        xmlChar* tmp;
                        max *= 2;
    ...
    if (uri->query != NULL) {
        // TODO: OPTIMIZE: combine Ifs (into one loop body)
        if (len + 3 >= max) {
            xmlChar* tmp;
            max *= 2;
            tmp = (xmlChar*) xmlRealloc(ret, (max + 1) * sizeof(xmlChar)); // DONE: Fix xmlRealloc
    ...
    if (uri->fragment != NULL) {
        ...
        ret[len++] = '#';
        p = uri->fragment;
        // TODO: OPTIMIZE: Yet another IFs to roll
        while (*p != 0) {
            if (len + 3 >= max) {
                xmlChar* tmp;
                max *= 2;
...
xmlParseURIScheme(xmlURIPtr uri, const char **str) {
    ...
    cur = *str;
    if (!IS_ALPHA(*cur))
        return(1); // 2 is equal to XML_ERR_NO_MEMORY and couse errors
    cur++;
    while (IS_SCHEME(*cur))
        cur++;
    if (uri != NULL) {
...
// XMLENGINE: END REPLACE
        // TODO: OPTIMIZE: Roll-in two loops
 ...
 *
 * OOM: possible --> returns NULL for uri!=NULL and sets OOM flag
 */
xmlURIPtr
xmlParseURI(const char *str) {
    ...
    if (str == NULL)
        return(NULL);
    uri = xmlCreateURI(); // may set OOM flag
    if (uri != NULL) {
        ret = xmlParseURIReference(uri, str); // may set OOM
        if (ret) {
            xmlCleanURI(uri);
            xmlFreeURI(uri);
            return(NULL);
...
#define IS_WINDOWS_PATH(p)                          \
    ((p != NULL) &&                                 \
     (((p[0] >= 'a') && (p[0] <= 'z')) ||           \
      ((p[0] >= 'A') && (p[0] <= 'Z'))) &&          \
     (p[1] == ':') && ((p[2] == '/') || (p[2] == '\\')))
//ISSUE: Not finished reviewing for OOM handling / OK: 16.05.05
xmlChar*
xmlCanonicPath(const xmlChar *path)
{
    ...
    {
        xmlFreeURI(uri);
        return xmlStrdup(path); // TODO: OOM:
    }
    // It's shpouild be OOM already!!! // ISSUE: Not finished work / OK: 16.05.05
    uri = xmlCreateURI();
    if (uri == NULL) {
        return(NULL);
    }
...
#else
    uri->path = (char *) xmlStrdup((const xmlChar *) path); //TODO: OOM:
#endif
