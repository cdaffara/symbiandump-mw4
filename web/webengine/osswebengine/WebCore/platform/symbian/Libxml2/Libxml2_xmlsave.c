 ...
 */
//static
// TODO: Optimize: Too much memory is allocated during saves
void
...
xmlNodeDumpOutputInternal(xmlSaveCtxtPtr ctxt, xmlNodePtr cur) {
    ...
    if(filterData)
        ...
        switch(filterData->proxyFn(filterData->fn, cur))
            {
            // TODO: Why KFilterReject is not constant for compiler here???
            //       Use constants.
    ...
    {
        // TODO: DONE: use switch / rearrange condition checks AND use cached type value
        /* Disabled: 
        ...
        */
        ...
        if (cur->type == XML_PI_NODE) {
            // TODO: correct program flow / exclude redundant IF check
            if (cur->content != NULL) {
                xmlOutputBufferWrite(buf, 2, "<?");
                xmlOutputBufferWriteString(buf, (const char *)cur->name);
                if (cur->content != NULL) {
    ...
    if (format == 1) {
        tmp = cur->children;
        while (tmp != NULL) { // TODO: OPTIMIZE: preevaluate
    ...
    if(skipMode != 2){
#endif
    // TODO: FIX: cur is not element for sure -- correct the condition -- is it needed?
    if ((cur->type != XML_ELEMENT_NODE) &&
        (cur->content != NULL))
    {
        xmlOutputBufferWriteEscape(buf, cur->content, ctxt->escape);
...
// TODO: Disable for Symbian builds
/**
 * xmlElemDump:
...
xmlElemDump(FILE * f, xmlDocPtr doc, xmlNodePtr cur)
{
    ...
    if (outbuf == NULL)
    {   // OOM!!!
        // TODO: OOM: Set OOM flag here or make this function return error code
        SET_OOM_FLAG;
        return;
    }
