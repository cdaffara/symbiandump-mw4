...
static String KeyIdentifierForKeyEvent(TInt code)
    {
    switch (code)
        {
...
/*  case NSF25FunctionKey:
    ...
    case NSUserFunctionKey:*/
        ...
        default:
            return String::format("U+%06X", toupper(code));
        }
    return "";
    }
...
static int WindowsKeyCodeForKeyEvent(TKeyEvent aEvent)
    ...
    */
    ...
    // Workaround to provide MSK keycode to JavaScript "keypres" event since MSK KeyEvent does not reach to BrowserEngine.
    if((aEvent.iCode == 0) && (aEvent.iScanCode==EStdKeyDevice3))
        return EKeyDevice3;
...
PlatformKeyboardEvent::PlatformKeyboardEvent( TKeyEvent event, TEventCode eventCode, bool forceAutoRepeat ) :
    ...
    , m_ctrlKey( event.iModifiers & EModifierCtrl )
    , m_altKey( event.iModifiers & EModifierAlt )
    , m_metaKey( EFalse ) // FIXME: Is this right?
    , m_symbianEvent(event)        
    {
