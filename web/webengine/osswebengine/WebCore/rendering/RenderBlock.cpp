...
#if PLATFORM(SYMBIAN)
    //Notify the view of the writing direction
    if (isBody())
        view()->frameView()->frame()->bridge()->setWritingDirectionRtl(style()->direction() == RTL);
#endif
...
#if PLATFORM(SYMBIAN)
        IntRect frameRect(view()->frameView()->contentsX(), view()->frameView()->contentsY(),
            view()->frameView()->width(), view()->frameView()->height());
