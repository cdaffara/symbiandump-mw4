...
#if PLATFORM(SYMBIAN)
        // display both alt text and broken image
        imageHeight+= min(font.height(), maxAltTextHeight);
#else
        ...
        if (cWidth > 2 && cHeight > 2) {
            ...
            if (errorOccurred() && !image()->isNull() && (usableWidth >= image()->width()) && (usableHeight >= image()->height())) {
                ...
                if (centerX < 0)
                    centerX = 0;
#if PLATFORM(SYMBIAN)
                // push image to the bottom to make space for alt text. centerY is a little bit misleading here
                int centerY = usableHeight - image()->height();
#else
