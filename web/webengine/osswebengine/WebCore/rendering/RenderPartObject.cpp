...
#if PLATFORM(SYMBIAN)  
    else if (element()->hasTagName(bgsoundTag)) {      
    
        HTMLBgsoundElement *o = static_cast<HTMLBgsoundElement*>(element());
        url = o->url;
...
#if PLATFORM(SYMBIAN)
bool RenderPartObject::isFocusable() const
{
    if (m_widget)
        return m_widget->isFocusable();
    return false;
}
