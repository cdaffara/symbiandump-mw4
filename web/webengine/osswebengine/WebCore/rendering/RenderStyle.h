...
#if PLATFORM(SYMBIAN)
//fixme CW has problems with references to bool, enum types
#define compareEqual(a,b) ((a)==(b))
#else
...
#if PLATFORM(SYMBIAN)
//------------------------------------------------
// Wap input format.  Not inherited

class StyleWapInput : public Shared<StyleWapInput>
{
public:
    StyleWapInput() : required(false)   {}
    StyleWapInput(const StyleWapInput& o) : Shared<StyleWapInput>(), format(o.format), required(o.required)  {}
    ~StyleWapInput() {}

    bool operator==(const StyleWapInput& o) const{
        return format == o.format && required == o.required;
    }
    bool operator!=(const StyleWapInput &o) const {
        return !(*this == o);
    }
...
#if PLATFORM(SYMBIAN)
    DataRef<StyleWapInput> wapInput;
#endif
...
#if PLATFORM(SYMBIAN)
    const AtomicString& wapInputFormat() const { return rareNonInheritedData->wapInput->format; }
    bool wapInputRequired() const { return rareNonInheritedData->wapInput->required; }
#endif
    // End CSS3 Getters
...
#if PLATFORM(SYMBIAN)
    void setWapInputFormat(String f) { SET_VAR(rareNonInheritedData.access()->wapInput, format, f); }
    void setWapInputRequired(bool b) { SET_VAR(rareNonInheritedData.access()->wapInput, required, b); }
#endif
    // End CSS3 Setters
