...
#if PLATFORM(SYMBIAN)
    OOM_PRE_CHECK(sizeof(RootInlineBox)*(cMaxLineDepth+1), 0, "RenderBlock::createLineBoxes")
#endif        
    do {
...
#if PLATFORM(SYMBIAN)
    OOM_POST_CHECK_FAILED( return ( static_cast<RenderFlow*>(this)->lastLineBox()); )
#endif
    return result;
}
...
#if PLATFORM(SYMBIAN)
                    // small screen, justify based on screen width
                    RenderView* v = view();
                    if (v && v->frameView() && v->frameView()->frame() && v->frameView()->frame()->bridge()) {
...
#if PLATFORM(SYMBIAN)
    //get screen width from the view
    RenderView* v = view();
    if (v && v->frameView() && v->frameView()->frame() && v->frameView()->frame()->bridge()) {
        int maxTextWidth = v->frameView()->frame()->bridge()->maxBidiWidth();
        maxTextWidth = max(maxTextWidth - leftOffset(m_height), 0);
